...
private function UpdateRecord($name_f, $name, $desc, $data, $time, $doctor,$phone){
	...
	private function UpdateUser($login_f, $login, $password, $name, $img){
		...
		if(!empty($password)){
			$this->db->query("UPDATE `med_pacient` SET `login` = '$login', `pass` = '$password', `name` = '$name', `img` = '$img' WHERE `login` = '$login_f'");
		}else{
			$this->db->query("UPDATE `med_pacient` SET `login` = '$login', `name` = '$name', `img` = '$img',  WHERE `login` = '$login_f'");
		}
	}
